...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            # print("time difference",time.time() - time1)
            ...
            if z < goal_bias:
                ...
                if (state_dim == 2):
                    x_rand = np.array([np.random.uniform(-5,5), np.random.uniform(-5,5)])
                else:
                    x_rand = np.array([np.random.uniform(-5,5), np.random.uniform(-5,5), np.random.uniform(self.statespace_lo[2] ,self.statespace_hi[2])])

            ind = self.find_nearest(V[range(n),:],x_rand)
            x_near = V[ind,:]
            ...
            if self.is_free_motion(self.obstacles,x_near,x_new):
                V[n,:] = x_new
                P[n] = ind
                if all(x_new == self.x_goal) == True:
                    self.path.append(x_new)
                    while True:
                        x_par = V[P[n],:]
                        # print("x_par", x_par)
                        n = P[n]
                        self.path.append(x_par)
                        if n == 0:
                            self.path.reverse()
                            success = True
                            break
    ...
    def shortcut_path(self):
        ...
        """
        ########## Code starts here ##########
        # x_prev = np.zeros(len(self.path[0]))
        success = False
        while success == False:
            success = True
            n = len(self.path)
            for x in range(n-1):
                # print("current node",self.path[x])
                if (all(self.path[x] == self.x_goal) == False) & (all(self.path[x] == self.x_init) == False):
                    if (self.is_free_motion(self.obstacles,self.path[x-1],self.path[x+1]) == True):
                        path_new = [a for a in self.path if all(a != self.path[x])] 
                        self.path = path_new
                        success = False
                        break
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        return (np.minimum(np.linalg.norm(x2 - x1),eps)*(x2-x1)*(1/np.linalg.norm(x2 - x1))) + x1

        # return (np.minimum(np.linalg.norm(x2 - x1),eps)*(1/np.linalg.norm(x2 - x1)))*x2
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        for i in range(V.shape[0]):
            x_curr = V[i,:]
            d_path = dubins.shortest_path(x_curr,x,self.turning_radius)
            path_len = d_path.path_length()
            if path_len < len_prev:
                index = i
                len_prev = path_len
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        ########## Code starts here ##########
        d_path = dubins.shortest_path(x1,x2,self.turning_radius*1.001)
        path_len = d_path.path_length()

        if path_len < eps or all(x2 == self.x_goal): 
            return x2
        else: 
            pts = d_path.sample_many(eps)[0]
            x2 = pts[1]
            return x2
